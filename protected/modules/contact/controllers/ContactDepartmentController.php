...
class ContactDepartmentController extends Controller
{
	...
	public function actionCreate()
	{
		...
		{
			$model->attributes=$_POST['ContactDepartment'];
			if($model->save())
			{
				Yii::app()->user->setFlash('success','بخش جدید اضافه گردید!');
				$this->refresh();
			}
			else
	...
	{
		$model=new ContactDepartment('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['ContactDepartment']))
			$model->attributes=$_GET['ContactDepartment'];
	 ...
	 * @return ContactDepartment the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=ContactDepartment::model()->findByPk($id);
		if($model===null)
	 ...
	 * @param ContactDepartment $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='contact-department-form')
		{
